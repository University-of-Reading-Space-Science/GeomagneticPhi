...
osffilepath = os.path.join(root, 'data','OSF_OEL2023.csv')
oulu_phi_path = os.path.join(root, 'data','Phi_Table_2017.txt')
hcs_file_path = os.path.join(root, 'data', 'WSO - Computed _Tilt_ Angle of the HCS.html')
...
#limit period of study to Krivova data
# mask = ~np.isfinite(osf_df['OSF_SSN_kriv'] )
# osf_df.loc[mask,'OSF_SSN'] = np.nan
# osf_df.loc[mask,'OSF_GEO'] = np.nan


#load the NM modulation potential data
nm_hmp = sunspots.load_oulu_phi(filepath = oulu_phi_path, download_now = updatenow)
...
#load the hcs tilt
hcstilt =  sunspots.load_wsa_hcstilt(filepath = hcs_file_path, download_now = updatenow)
...
#ax.plot(osf_df['datetime'], osf_df['OSF_SSN_kriv'], 'g', label = 'SN_kr')
...
non_nan_indices = osf_df[osf_df['OSF_SSN'].notna() & 
                         ...
                         osf_df['OSF_OMNI'].notna() 
                         #osf_df['OSF_SSN_kriv'].notna()
                         ].index
...
print("OMNI, WSO")
print("Best-fit parameters, AU2016:", best_fit_params)
...
xx_ts = ax.get_xlim()
ax.set_xlim(xx_ts)
ax.plot(osf_df['datetime'], phi_E2016_omni,'b', label = 'OMNI, WSO (AU2016)')
...
ax.set_xlabel(r'$\phi$ OMNI, WSO (AU2016) [MV]')
...
xx_ts = ax.get_xlim()
ax.set_xlim(xx_ts)
ax.plot(osf_df['datetime'], phi_E2016_omni,'b', label = 'OMNI (AU2016)')
ax.legend(loc = 'lower left')
yy_ts = ax.get_ylim()
ax.set_ylim(yy_ts)
ax.set_ylabel(r'$\phi$ [MV]')
...
ax.set_title(r'$r_L$ = ' + "{:.3f}".format(rE16omni) + r'; MAE = ' + "{:.2f}".format(maeE16omni))
ax.set_ylabel(r'$\phi$ NM [MV]')
ax.set_xlabel(r'$\phi$ OMNI (AU2016) [MV]')
ax.set_ylim(xx_sc); ax.set_xlim(xx_sc)
#plt.title(r'$r_L$ = ' + "{:.2f}".format(rl[0,1]), fontsize=12) 
ax.plot(xx_sc,xx_sc,'k--')
ax.text(0.05, 0.95, '(b)', transform=plt.gca().transAxes, fontsize=fontsize)
...
xx_ts = ax.get_xlim()
ax.set_xlim(xx_ts)
ax.plot(osf_df['datetime'], phi_E2016_geo,'b', label = 'GEO (AU2016)')
ax.legend()
yy_ts = ax.get_ylim()
ax.set_ylim(yy_ts)
ax.set_ylabel(r'$\phi$ [MV]')
...
ax.set_ylabel(r'$\phi$ NM [MV]')
ax.set_xlabel(r'$\phi$ GEO (AU2016) [MV]')
ax.set_ylim(xx_sc); ax.set_xlim(xx_sc)
#plt.title(r'$r_L$ = ' + "{:.2f}".format(rl[0,1]), fontsize=12) 
ax.plot(xx_sc,xx_sc,'k--')
ax.text(0.05, 0.95, '(b)', transform=plt.gca().transAxes, fontsize=fontsize)
...
ax = plt.subplot(2,2,4)
ax.plot(phi_new_geo, phi, 'ro')
rnew_geo, nnew_geo = mplot.lin_correl(phi_new_geo, phi)
maenew_geo = np.nanmean(abs(phi_new_geo - phi))
...
#fit_slope, fit_intercept, conf_slope, conf_intercept =  mplot.lin_regress_bootstrap(phi_E2016_geo, phi)
pE2016_sn, regressparamsE2016sn = mplot.lin_regress(phi_E2016_sn, phi, 
                                               plotnow = False, confid_level = confid_level)
confid_int_E2016_sn, prob_int_E2016_sn = mplot.lin_regress_confid_int(phi_E2016_sn, regressparamsE2016sn)
...
xx_ts = ax.get_xlim()
ax.set_xlim(xx_ts)
ax.plot(osf_df['datetime'], phi_E2016_sn,'b', label = 'SN (AU2016)')
ax.legend()
yy_ts = ax.get_ylim()
ax.set_ylim(yy_ts)
ax.set_ylabel(r'$\phi$ [MV]')
...
ax = plt.subplot(2,2,2)
ax.plot(phi_E2016_sn, phi, 'ro')
rE16_sn, n = mplot.lin_correl(phi_E2016_sn, phi)
maeE16_sn = np.nanmean(abs(phi_E2016_sn - phi))
ax.set_title(r'$r_L$ = ' + "{:.3f}".format(rE16_sn) + r'; MAE = ' + "{:.2f}".format(maeE16_sn))
ax.set_ylabel(r'$\phi$ NM [MV]')
ax.set_xlabel(r'$\phi$ SN (AU2016) [MV]')
ax.set_ylim(xx_sc); ax.set_xlim(xx_sc)
#plt.title(r'$r_L$ = ' + "{:.2f}".format(rl[0,1]), fontsize=12) 
ax.plot(xx_sc,xx_sc,'k--')
ax.text(0.05, 0.95, '(b)', transform=plt.gca().transAxes, fontsize=fontsize)
...
pnew_sn, regressparamsnewsn = mplot.lin_regress(phi_new_sn, phi, 
                                               plotnow = False, confid_level = confid_level)
confid_int_new_sn, prob_int_new_sn = mplot.lin_regress_confid_int(phi_new_sn, regressparamsnewsn)
...
rnew_sn, n = mplot.lin_correl(phi_new_sn, phi)
maenew_sn = np.nanmean(abs(phi_new_sn - phi))
...
phi_df['phi_geo_AU2016 [MV]'] = phi_E2016_geo
phi_df['phi_geo_AU2016_min [MV]'] = phi_E2016_geo - prob_int_E2016
phi_df['phi_geo_AU2016_max [MV]'] = phi_E2016_geo + prob_int_E2016
phi_df['phi_geo [MV]'] = phi_new_geo
phi_df['phi_geo_min [MV]'] = phi_E2016_geo - prob_int_new
phi_df['phi_geo_max [MV]'] = phi_E2016_geo + prob_int_new
...
#save the data
outputfilepath = os.path.join(root, 'data','HMP_GEO_O2024.csv')
...
phi_IC = sunspots.load_oulu_phi_extended(filepath = phi_IC_filepath, download_now = updatenow)
#sample to 1y
...
#also load teh QL estimate that is reproduced in Brehm 2021
carbonQL = sunspots.load_14C_QL_phi(filepath = phi_14C_QL_filepath)
mask_before_suess_QL = carbonQL['datetime'] <= datetime(suess_start,1,1)
mask_after_suess_QL = carbonQL['datetime'] >= datetime(suess_start-1,1,1)

# plt.figure()
# plt.plot(carbon['datetime'], carbon['phi_14C'],'k')
# plt.plot(carbonQL['datetime'], carbonQL['phi_14C'],'r')
