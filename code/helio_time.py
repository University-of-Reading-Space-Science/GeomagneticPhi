...
def date2jd(*args):
    ...
    *optional
    ...
    Modified to use vectorisation on 15/8/23
    ...
    vec_date2jd = np.vectorize(_date2jd_)
    jd = vec_date2jd(*args) 
...
def doyyr2fracyear(doy, yr):
    """
    Converts (decimal) day-of-year and (integer) year to decimal year
    Mathew Owens, 15/8/23
    """ 
    def _create_fracyear_(doy, yr):
        if isleapyear(yr):
            assert((doy <= 367) & (doy >= 1))
            fracyr = (doy-1)/366
        else:
            assert((doy <= 366) & (doy >= 1))
            fracyr = (doy-1)/365
        return fracyr + yr
    vec_create_fracyear = np.vectorize(_create_fracyear_)
    fracyr = vec_create_fracyear(doy,yr) 
...
def mjd2fracyear(mjd):
    """
    Converts MJD to decimal year
    Mathew Owens, 15/8/23
    """ 
    
    def _mjd2fracyear_(mjd):
        doy, yr = mjd2doyyr(mjd)
        fracyr = doyyr2fracyear(doy, yr)
        return fracyr 
    
    vec_mjd2fracyear = np.vectorize(_mjd2fracyear_)
    fracyear = vec_mjd2fracyear(mjd) 
